  ...
  methods: {
    ...
    async saveEdit() {
      ...
      try {
        const response = await axios.patch(
            `https://api.donggurami.net/club-leader/${clubUUID}/members/${memberUUID}/non-member`,
            updateData,
            {
              headers: {
    ...
    async fetchData() {
      ...
      try {
        const [regularResponse, nonRegularResponse] = await Promise.all([
          axios.get(`https://api.donggurami.net/club-leader/${clubUUID}/members?sort=regular-member`, {
            headers: {
              'Authorization': `Bearer ${accessToken}`,
              'Content-Type': 'application/json'
            }
          }),
          axios.get(`https://api.donggurami.net/club-leader/${clubUUID}/members?sort=non-member`, {
            headers: {
              'Authorization': `Bearer ${accessToken}`,
              'Content-Type': 'application/json'
            }
    ...
    async sheetDownload(){
      ...
      try {
        ...
        const response = await axios.get(`https://api.donggurami.net/club-leader/${clubUUID}/members/export`, {
