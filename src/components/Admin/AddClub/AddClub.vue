  ...
  methods: {
    ...
    async idDuplicateCheck() { //아이디 중복 확인
      if(this.id !== ''){
        ...
        try {
          await axios.get(`https://api.donggurami.net/admin/clubs/leader/check?leaderAccount=${this.id}`, {
            headers: {
    ...
    async clubNameDuplicateCheck() { //동아리 이름 중복 확인
      if(this.clubName !== '') {
        const accessToken = store.state.accessToken; // 저장된 accessToken 가져오기
        try {
          await axios.get(`https://api.donggurami.net/admin/clubs/name/check?clubName=${this.clubName}`, {
            headers: {
              'Authorization': `Bearer ${accessToken}`, // 헤더에 accessToken 추가
            }
          });
